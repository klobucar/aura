...
lazy_static! {
    static ref URL_REGEX: Regex = Regex::new(r"https?://(?:www\.)?([a-zA-Z0-9.-]+\.[a-z]{2,})[^\s]*").unwrap();
...
mod tests {
    ...
    fn test_sanitize_urls() {
        ...
        // Brand mapping
        assert_eq!(formatter.sanitize("Check this out: https://youtube.com/watch?v=123"), "Check this out: YouTube link");
        
        // Short domain
        assert_eq!(formatter.sanitize("Go to http://example.com/foo"), "Go to link to example.com");
        
        // Long domain fallback
        assert_eq!(formatter.sanitize("Visit http://very-long-and-super-suspicious-domain-name-that-is-too-long.com/path"), "Visit link");
        
        // Multiple URLs
        assert_eq!(formatter.sanitize("Search on https://google.com or code on https://github.com"), "Search on Google link or code on GitHub link");
    }
}
