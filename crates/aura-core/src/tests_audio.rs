...
use crate::uniffi_bindings::{AudioSenderWrapper, AudioReceiverWrapper};
use std::sync::Arc;
...
async fn test_full_audio_pipeline() {
    ...
    assert!(mixed.active_speakers.contains(&1), "Sender 1 should be an active speaker");
}
...
fn test_concurrent_senders() {
    ...
    for session_id in 1..=5 {
        ...
        receiver.add_sender(session_id, &key, 0).expect("Failed to add sender");
...
fn test_zero_amplitude_handling() {
    ...
    let max_amplitude = mixed.pcm.iter().map(|&s| (s as i32).abs()).max().unwrap_or(0);
    assert!(max_amplitude < 50, "Decoded silence should be near zero, got {}", max_amplitude);
}
