...
impl Opus16Encoder {
    ...
    ) -> Result<Self, Opus16Error> {
        let fs = validate_sample_rate(sample_rate)?;
        let ch = validate_channels(channels)?;

        let mut error: c_int = 0;
        let ptr = unsafe { ffi::opus_encoder_create(fs, ch, application.to_ffi(), &mut error) };
...
impl Opus16Decoder {
    ...
    pub fn new(sample_rate: u32, channels: u8) -> Result<Self, Opus16Error> {
        ...
        let mut error: c_int = 0;
        let ptr = unsafe { ffi::opus_decoder_create(fs, ch, &mut error) };
...
mod tests {
    ...
    fn test_invalid_sample_rate() {
        ...
        assert!(matches!(
            encoder,
            Err(Opus16Error::InvalidSampleRate(44100))
        ));
    }
