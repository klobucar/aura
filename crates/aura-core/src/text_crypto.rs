...
use crate::crypto::{CryptoError, DaveCrypto, NONCE_SIZE};
use prost::Message;

/// Re-export text message types from protocol
pub use aura_protocol::{EncryptedTextPacket, TextMessage};
...
pub fn decrypt_text(
    ...
    let nonce: [u8; NONCE_SIZE] = packet
        .nonce
        .as_slice()
        .try_into()
    ...
    TextMessage::decode(plaintext.as_slice()).map_err(|_| CryptoError::DecryptionFailed)
}
...
mod tests {
    ...
    fn test_encrypt_decrypt_roundtrip() {
        ...
        let packet =
            encrypt_text(&key, 1, "100".into(), 1, &message).expect("Encryption should succeed");

        let decrypted = decrypt_text(&key, &packet).expect("Decryption should succeed");
    ...
    fn test_wrong_key_fails() {
        ...
        let packet =
            encrypt_text(&key1, 1, "100".into(), 1, &message).expect("Encryption should succeed");
    ...
    fn test_packet_metadata() {
        ...
        let packet =
            encrypt_text(&key, 5, "42".into(), 7, &message).expect("Encryption should succeed");
